...
reg_grid_samples = {}
coords = itertools.product(np.arange(0, lat_end, lat_freq), np.arange(0, lon_end, lon_freq))
for year in np.arange(10):
    coords = itertools.product(np.arange(lat_start, lat_end, lat_freq), np.arange(lon_start, lon_end, lon_freq))
    reg_grid_samples[year] = []
    for x in coords:
        #print ma.mean(year_stack[year, :, :, ], 1)[x]
        reg_grid_samples[year].append(ma.mean(year_stack[year, :, :, ], 1)[x])
...
coords = itertools.product(np.arange(lat_start, lat_end, lat_freq), np.arange(lon_start, lon_end, lon_freq))
for item in coords:
    samples.append(decadal_mean[item])
...
filename = os.getcwd() + '/output/tex/reg_grid/annual_means-'+str(np.round(original_fitness, 2))+ '_'+str(lat_freq)+'_'+str(lon_freq)+'.tex'
